...
window.goToGame = function(mode) {
  ...
  if (gameTypes.includes(mode)) {
    // 确保用户数据已保存
    UserService.updateUser(UserService.getUser());
...
window.logout = function() {
  console.log("用户退出登录");
  // 重置为默认用户
  UserService.updateUser(UserService.createDefaultUser());
  window.location.href = 'login.html';
}
